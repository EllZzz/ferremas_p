...
export default function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:5173/api/products') // Ajusta la URL a tu backend
      .then(res => res.json())
          ...
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {products.map(product => (
                <div key={product.idProduct} className="border p-4 rounded-lg shadow hover:shadow-lg transition">
                  <img
                  ...
                  <p className="text-blue-800 font-bold">${product.product_unitprice}</p>
                </div>
              ))}
            </div>
          </div>
